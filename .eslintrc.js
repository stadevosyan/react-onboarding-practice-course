...
module.exports = {
    ...
    plugins: ['import', 'folder-schema', '@servicetitan/processors-stub', 'react-hooks'],
...
function moduleZones() {
    ...
    const zones = modules.map(name => zone(base, name, ['common']));
...
function subModulesZones(base, levels) {
    ...
    const zones = subModules.map(name => zone(base, name, byFileType));
    return [...zones, ...subZones()];
...
function zone(base, dirName, allowedSiblings = []) {
    const allowedImports = [dirName, ...allowedSiblings];
